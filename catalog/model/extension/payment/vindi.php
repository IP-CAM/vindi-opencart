...
class ModelExtensionPaymentVindi extends Model
{
    ...
    public function getMethod($address, $total)
    {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('payment_vindi_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

        if ($this->config->get('payment_vindi_total') > 0 && $this->config->get('payment_vindi_total') > $total) {
            $status = false;
        } elseif (!$this->config->get('payment_vindi_geo_zone_id')) {
            $status = true;
    ...
    {
        $transaction_result = $this->db->query("SELECT * FROM `" . DB_PREFIX . "vindi_transaction` WHERE `transaction_id`='" . $this->db->escape($transaction_id) . "' AND `order_id`='" . (int)$order_id . "'");
    ...
    {
        $this->db->query("UPDATE `" . DB_PREFIX . "vindi_transaction` SET `merchant`='" . $this->db->escape($data['merchant']) . "', `partnerSolutionId`='" . $this->db->escape($data['partnerSolutionId']) . "', `response_gatewayCode`='" . $this->db->escape($data['response']['gatewayCode']) . "', `result`='" . $this->db->escape($data['result']) . "', `transaction_type`='" . $this->db->escape($data['transaction']['type']) . "', `transaction_amount`='" . (float)$data['transaction']['amount'] . "', `transaction_currency`='" . $this->db->escape($data['transaction']['currency']) . "', `billing_address_city`='" . $this->db->escape($data['billing']['address']['city']) . "', `billing_address_company`='" . $this->db->escape($data['billing']['address']['company']) . "', `billing_address_country`='" . $this->db->escape($data['billing']['address']['country']) . "', `billing_address_postcodeZip`='" . $this->db->escape($data['billing']['address']['postcodeZip']) . "', `billing_address_stateProvince`='" . $this->db->escape($data['billing']['address']['stateProvince']) . "', `billing_address_street`='" . $this->db->escape($data['billing']['address']['street']) . "', `risk_response_gatewayCode`='" . $this->db->escape($data['risk']['response']['gatewayCode']) . "', `risk_response_totalScore`='" . (int)$data['risk']['response']['totalScore'] . "', `version`='" . $this->db->escape($data['version']) . "', `device_browser`='" . $this->db->escape($data['device']['browser']) . "', `device_ipAddress`='" . $this->db->escape($data['device']['ipAddress']) . "', `timeOfRecord`='" . $this->db->escape($data['timeOfRecord']) . "', `notification_date`=NOW() WHERE `transaction_id`='" . $this->db->escape($data['transaction']['id']) . "' AND `order_id`='" . (int)$data['order']['id'] . "'");
    ...
    public function saveToken($order_id, $token, $session_id, $sourceOfFunds)
    {
        ...
        if (!empty($response['result']) && $response['result'] == 'SUCCESS' && $response['status'] == 'VALID') {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "vindi_token` SET `customer_id`='" . (int)$customer_id . "', `token`='" . $this->db->escape($token) . "', `date_added`=NOW()");
        } else {
            ...
            if (!empty($response_second['result']) && $response_second['result'] == 'SUCCESS' && $response_second['status'] == 'VALID') {
                $this->db->query("INSERT INTO `" . DB_PREFIX . "vindi_token` SET `customer_id`='" . (int)$customer_id . "', `token`='" . $this->db->escape($response_second['token']) . "', `date_added`=NOW()");
            }
        }
    }
    ...
    public function getCards($customer_id)
    {
        ...
        foreach ($this->getTokens($customer_id) as $token_data) {
            ...
            if (
                ...
                $this->db->query("DELETE FROM `" . DB_PREFIX . "vindi_token` WHERE vindi_token_id='" . (int)$token_data['vindi_token_id'] . "'");
            } else {
                $result[] = [
    ...
    {
        $tokens_result = $this->db->query("SELECT * FROM `" . DB_PREFIX . "vindi_token` WHERE customer_id='" . (int)$customer_id . "'");
    ...
    public function initCheckoutSession($order_id, $currency)
    {
        ...
        $data['order']['notificationUrl'] = $this->url->link('extension/payment/vindi/callback', '', true);

        $response = $this->api('session', $data, 'POST');

        if (!empty($response['result']) && $response['result'] == 'SUCCESS') {
            $this->session->data['vindi'] = $response;
    ...
    {
        unset($this->session->data['vindi']);
    }

    public function addCustomer()
    {
        unset($this->session->data['vindi']);
        if (empty($customerVindi = $this->api('customers/?page=1&query=code%3D' . $this->customer->getId())['customers'][0])) {
            $customerVindi = '';
        }
    ...
    public function createBill(array $customer, float $shipping)
    {
